...
function Input_Digits (digit) {
    const {Display_Value, Wait_Second_Operand } = Calculator;
    //This checks if the Wait_Second_Operand is true and sets Display_Value
    //to the key that wasa cliked on 
    if (Wait_Second_Operand === true) {
        Calculator.Display_Value =digit;
        Calculator.Wait_Second_Operand = false;
    } else {
        //This overides Display_Value if the current value is 0
...
function Handle_Operator(Next_Operator) {
    ...
    //then updates the operator an exit from function.
    ...
    } else if (operator) {//Checks if an oparator already exists
        const Value_Now = First_Operand || 0;
        // If operator exists, property lookup is performed for the operator
        //in the Perform_Calculation object and the function that matches the 
        //operator is executed.
        let result = Perform_Calculation[operator](Value_Now, Value_of_Input);
        //Here we add a fixed amount after decimal number
...
keys.addEventListener('click', (event) => {
    //The target variabe is an object that represents the element
    //that was clicked
    const {target} = event;
    //if tghe element that waw clicked on is not a button, exit the function
